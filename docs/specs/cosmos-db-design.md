...
プラットフォーム管理 7 コンテナ + MAF 自動作成 1 コンテナ（messages）のフィールド定義、パーティションキー設計、インデキシング、同時実行制御、スループット戦略を網羅する。
...
├── Database: "maf"
│   ├── agents                       ← Spec 系（共有スループット）
│   ├── tools                        ← Spec 系（共有スループット）
...
|---|-----------|------|-----------------|-----|------|
| 1 | `agents` | Spec | `/id` | — | エージェント仕様（モデル・指示・ツール構成） |
| 2 | `tools` | Spec | `/id` | — | ツール仕様（関数定義・パラメータスキーマ） |
...
- **Spec 系** (`agents`, `tools`, `workflows`): 件数が限定的（数百件以下）のため、PK `/id` でポイントリード（1 RU）を最優先。一覧系クエリのクロスパーティションコストは許容する。件数が 1,000 件を超える場合は PK 再設計を検討。
...
erDiagram
    ...
    users ||--o{ agents : "作成者"
    users ||--o{ tools : "作成者"
    agents }o--o{ tools : "ツール構成"
    agents ||--o{ sessions : "Agent との会話"
    agents ||--o{ workflow_execution_steps : "agentId で参照"
...
MAF の `ChatAgent` 構築に必要な設定を永続化する。`spec_management` サービスで CRUD し、実行時に `PlatformAgentFactory` で `ChatAgent` インスタンスに変換する。
...
| 7 | ツール構成 | `toolIds` | array\<string\> | ○ | `tools.id` の配列。実行時に解決 |
...
> **コード定義との関係**: `agents/` にコードとして定義された Agent も、起動時に `agents` に登録（upsert）する運用を想定。API からの動的登録と共存可能。
...
| 2 | セッションID | `sessionId` | string | — | パーティションキー兼用。`AgentSession.session_id` と同値を共有。`id` と同値 |
| 3 | ユーザーID | `userId` | string | — | セッション所有者 |
| 4 | Agent仕様ID | `agentId` | string | — | どの Agent との会話か。`agents.id` への参照 |
| 5 | セッションステータス | `status` | string | — | active / closed / expired |
| 6 | タイトル | `title` | string | ○ | 会話タイトル（UI表示用、最初のメッセージから自動生成等） |
| 7 | 作成日時 | `createdAt` | string (ISO 8601) | — | |
| 8 | 更新日時 | `updatedAt` | string (ISO 8601) | — | |
| 9 | スキーマバージョン | `schemaVersion` | number | — | ドキュメントスキーマバージョン。初期値 `1` |
| 10 | TTL | `ttl` | number | ○ | Cosmos DB TTL（秒）。expired セッションの自動削除用 |
...
|------|------|
| パーティションキー | `/sessionId` |
| 外部参照 | `agentId` → `agents.id` |
| 外部参照 | `userId` → `users.id` |

> **AgentSession との対応**: MAF の `AgentSession` は `session_id`, `service_session_id`, `state` のみ持つ軽量コンテナ。Platform Session は `session_id` を MAF AgentSession と共有し、`userId`, `agentId`, `status`, `title` 等のメタデータを独自に管理する。`service_session_id` と `state` は MAF 側で管理するため Platform Session には持たない。

> **会話履歴**: MAF `CosmosHistoryProvider` が `messages` コンテナに永続化する。Platform は会話履歴の二重管理をしない。
...
| `sessions` | 推奨 | status / title の更新 | 同時更新保護 |
| `agents` | 推奨 | 全フィールド更新 | 複数管理者による同時 spec 更新の保護 |
| `tools` | 推奨 | 全フィールド更新 | 同上 |
...
| 開発/検証環境 | **Serverless** | 使った分だけ課金。8 コンテナでも最低料金不要 |
...
- **Source of Truth は `agents` コンテナ**（自前 Cosmos DB）(旧 `agent_specs`)
...
│                                                     │
│  agents ──→ spec_management ──→ foundry_sync   │
│  tools       (CRUD)             (push)         │
│  workflows                              │           │
└─────────────────────────────────────────┼───────────┘
                                          │
...
自前 `agents` → Foundry `POST /agents` で PromptAgentDefinition として登録。
...
- Session の `sessionId` を MAF `AgentSession.session_id` と共有
- Foundry 連携時は MAF `AgentSession.service_session_id` に Foundry Conversation ID を設定（MAF 側で管理、Platform Session には持たない）
- 会話履歴は MAF `CosmosHistoryProvider` が `messages` コンテナに保存
